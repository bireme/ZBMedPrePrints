...
import org.bireme.processing.tools.models.ZbmedDoc
...
class ZbmedToXml {
  ...
  val logger: Logger = LoggerFactory.getLogger(classOf[ZbmedToXml])

  def normalizeData(docsMongo: Seq[Document], pathOut: String): Try[Iterator[ZbmedDoc]] = {

    logger.info("+++Normalization process started")
    var index = 0
    val zbmedppIteratorList: Try[Iterator[ZbmedDoc]] = generateXml(docsMongo.map {
  ...
  private def mapElements(doc: Document): Try[ZbmedDoc] = {
    Try {
      ...
      ZbmedDoc(id, alternateId, db, instance, collection, typeTmp, la, fo, dp, pu, ti, aid, link, linkPdf, fullText, ab, au, entryDate, da, mj)
    }
  }
  ...
  private def generateXml(elements: Seq[ZbmedDoc], pathOut: String): Try[Iterator[ZbmedDoc]] = {
    Try {
  ...
  private def docToElem(fields: ZbmedDoc): Elem = {
