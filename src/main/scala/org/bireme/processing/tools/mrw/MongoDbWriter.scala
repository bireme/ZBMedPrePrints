...
class MongoDbWriter(params: mdwParameters) {
  ...
  private val documentCountBeforeProcess: Long = dbase.getCollection(params.collection).countDocuments()
  ...
  def renameColl(newNameColl: String): Unit = {
    coll.renameCollection(new MongoNamespace(newNameColl))
  }
  ...
  def getDocumentCountBeforeProcess(): Long ={
    documentCountBeforeProcess
  }
