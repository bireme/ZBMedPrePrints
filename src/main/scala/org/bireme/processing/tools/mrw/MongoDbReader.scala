...
class MongoDbReader(params: mdrParameters) {
  ...
  def renameColl(oldNameColl: String, newNameColl: String): Unit = {
    dbase.getCollection(oldNameColl).renameCollection(new MongoNamespace(params.database, newNameColl))
  }
  ...
  def countDocuments(collectionName: String = params.collection): Int ={
    dbase.getCollection(collectionName).countDocuments().toInt
  }

  def dropColl(collectionName: String = params.collection): Unit = {
    dbase.getCollection(collectionName).drop()
  }
